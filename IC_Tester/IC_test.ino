...
void repeatTest(const String& number, int numberTest)
{
  ...
  tft.setCursor((120 - ((((7+number.length())*12)-1)/2)), 100);
  ...
  for (uint8_t i = 0; i < 24; i++) {
    storeErrorPlace[i] = clearArray[0];  //Clear the Pin number error assignments
    storeErrorLines[i] = clearArray[0];
  }
  for (uint8_t i = 0; i < 24; i++) {
    storeTestLines[i] = ""; //need to change the dependency on 24 as max need to tie it to a reference
  }
    ...
    while (dataFile.available())
    {
      ...
      if (number == buffer)//Compares the IC number read from the SD to the IC number chosen by the user
      {
        ...
        *hardPositionPointer = dataFile.position(); //this keeps track of the start position of the chosen IC
        ...
        if (numberTest == 0) {
          numberTest = 10;  //this makes sure that the numberTest variable isn't 0 when entering the while loop
          switches.endlessLoopSwitch = 1;
        }
        //----------------------------------------------------------------------------------------------------------------------//
        //------------------------------------------------Testing starts--------------------------------------------------------//
        //----------------------------------------------------------------------------------------------------------------------//
        while (testLoop < numberTest * (linecount - 1)) //testloop is the number of times this loop has been executed and the numberTest is the number of tests chosen by the user
        {
          ...
          if (*testSequenceLenghtPointer == linecount) {
            *testSequenceLenghtPointer = 1; //this is so the test sequence starts over at the right position on the SD card
          }
...
void errorInfo()
{
    ...
    while (dataFile.available())
    {
      ...
      *testPositionPointer = dataFile.position(); //this variable gets set to start of each line of the test routines, here it´s set to the start of the test sequence
      ...
      if (newChip.num.toInt() != numberRouting.toInt()) {
        dataFile.seek(*testPositionPointer + 210); //this "scrolls/jumps" through the test lines
      }
  ...
  for (uint8_t u = 0; u < *pinNumberRoutingPointer; u++)
  {
    ...
    if (*pinNumberRoutingPointer == 14) {
      pinMode = getPinMode(PIN14[u]);
    }
    if (*pinNumberRoutingPointer == 16) {
      pinMode = getPinMode(PIN16[u]);
    }
    if (*pinNumberRoutingPointer == 20) {
      pinMode = getPinMode(PIN20[u]);
    }
    if (*pinNumberRoutingPointer == 24) {
      pinMode = getPinMode(PIN24[u]);
    }
    ...
    if (*globalReferencePointer[u] == "+V") {
      tft.setTextColor(RED);
    }
    if (*globalReferencePointer[u] == "0V") {
      tft.setTextColor(GREEN);
    }
    tft.setCursor(100, 10 + (10 * (u + 1))); tft.print(*globalReferencePointer[u]);

    //functiontype
    if (*globalpinFunctionPointer[u] == "Output ") {
      tft.setTextColor(YELLOW);
    }
    if (*globalpinFunctionPointer[u] == "Input  ") {
      tft.setTextColor(NAVY);
    }
    if (*globalpinFunctionPointer[u] == "VCC    ") {
      tft.setTextColor(RED);
    }
    if (*globalpinFunctionPointer[u] == "GND    ") {
      tft.setTextColor(GREEN);
    }
    if (*globalpinFunctionPointer[u] == "Clock  ") {
      tft.setTextColor(MAROON);
    }
    if (*globalpinFunctionPointer[u] == "In/Out ") {
      tft.setTextColor(ORANGE);
    }
    if (*globalpinFunctionPointer[u] == "Out/In ") {
      tft.setTextColor(PURPLE);
    }
    if (*globalpinFunctionPointer[u] == "NC     ") {
      tft.setTextColor(OLIVE);
    }
    tft.setCursor(30, 10 + (10 * (u + 1))); tft.print(*globalpinFunctionPointer[u]);

    //Errortypes
    if (storeErrorLines[u] == "L") {
      tft.setTextColor(PURPLE);
    }
    if (storeErrorLines[u] == "H") {
      tft.setTextColor(GREEN);
    }
    if (storeErrorLines[u] == "0") {
      tft.setTextColor(BLACK);
    }
...
void rawData()
{
  ...
  for (uint8_t t = 0; t < linecount; t++)
  {
    if (storeTestLines[t] != "")
    {
      ...
      {
        tft.setTextColor(YELLOW);
        if (storeErrorLines[r] == "H" && testLinesArray[r] == "L") {
          tft.setTextColor(RED);
        }
        if (storeErrorLines[r] == "L" && testLinesArray[r] == "H") {
          tft.setTextColor(RED);
        }
