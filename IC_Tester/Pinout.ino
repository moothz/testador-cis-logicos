...
void Test_init(int pins)
{
  ...
  tft.println(F("Pinagem de CIs"));
  
  tft.setCursor((120 - ((((7+numberRouting.length())*12)-1)/2)), 100);
  ...
  if (dataFile)
  {
    while (dataFile.available())
    {
      ...
      *testPositionPointer = dataFile.position(); //this variable gets set to start of each line of the test routines, here it´s set to the start of the test sequence
      ...
      if (newChip.num.toInt() != numberRouting.toInt()) {
        dataFile.seek(*testPositionPointer + 210); //this "scrolls/jumps" through the Pinout lines
      }
      ...
      if (newChip.num.toInt() == numberRouting.toInt()) //comparing IC number to the one chosen by the user
      {
        ...
        {
          if (newChip.pins == 14 || newChip.pins == 16) {
            tft.drawRect(80, 10 , 80, 240, BLUE); //chip outline if the IC is a 14 or 16 pin chip
          }
          if (newChip.pins == 20) {
            tft.drawRect(80, 5 , 80, 270, BLUE); //chip outline if the chip is a 20 pin IC
          }
          if (newChip.pins == 24) {
            tft.drawRect(80, 10 , 80, 270, BLUE); //chip outline if the chip is a 24 pin IC
          }
        ...
        for (uint8_t i = 0; i < newChip.pins; i++) {
          outputs[i] = clearArray[0];  //Clear output Array
        ...
        for (uint8_t a = 0; a < newChip.pins / 2; a++)
        {
          ...
          *globalpinFunctionPointer[a] = newChip.pinFunction;
          if (newChip.pinFunction == "Input  ") {
            pinMode(pin[a], OUTPUT);  //
            digitalWrite(pin[a], LOW);
            buttonsPinout[a].initButton(&tft, 80, nudge + 10, buttonSize, buttonSize, NAVY, BLUE, WHITE, "", 1);
            if (switches.circumventTFT == 0) {
              buttonsPinout[a].drawButton();
            }
          }
          if (newChip.pinFunction == "Clock  ") {
            clock[a] = 1;  //
            buttonsPinout[a].initButton(&tft, 80, nudge + 10, buttonSize, buttonSize, BLUE, PURPLE, WHITE, "C", 2);
            if (switches.circumventTFT == 0) {
              buttonsPinout[a].drawButton();
            }
          }
          if (newChip.pinFunction == "Output ") {
            outputs[a] = 1;
            tablet.sortedOutputs[a] = 1;
            if (switches.circumventTFT == 0) {
              tft.fillCircle(80, nudge + 9, outputSize, DARKGREY);
            }
          }
          if (newChip.pinFunction == "GND    ") {
            pinMode(pin[a], OUTPUT);  //
            digitalWrite(pin[a], LOW);
            if (switches.circumventTFT == 0) {
              tft.fillRect(70 + horizontalNudge, nudge , boxSize, boxSize, GREEN);
            }
          }
          if (newChip.pinFunction == "VCC    ") {
            pinMode(pin[a], OUTPUT);  //
            digitalWrite(pin[a], HIGH);
            if (switches.circumventTFT == 0) {
              tft.fillRect(70 + horizontalNudge, nudge , boxSize, boxSize, RED);
            }
          }
          if (newChip.pinFunction == "NC     ") {
            if (switches.circumventTFT == 0) {
              tft.fillRect(70 + horizontalNudge, nudge , boxSize, boxSize, NAVY); //
            }
          }
          if (newChip.pinFunction == "In/Out ") {
            inOut[a] = 1;  //
            pinMode(pin[a], OUTPUT);
            digitalWrite(pin[a], LOW);
            buttonsPinout[a].initButton(&tft, 80, nudge + 10, buttonSize, buttonSize, WHITE, DARKGREY, WHITE, "", 1);
            if (switches.circumventTFT == 0) {
              buttonsPinout[a].drawButton();
            } *globalpinFunctionPointer[a] = "Input  ";
          }
          if (newChip.pinFunction == "Out/In ") {
            outIn[a] = 1;  //here the Com ports are outputs
            outputs[a] = 1;
            tablet.sortedOutputs[a] = 1;
            if (switches.circumventTFT == 0) {
              tft.fillCircle(80, nudge + 9, outputSize, DARKGREY);
            } *globalpinFunctionPointer[a] = "Output  ";
          }
          ...
          *globalReferencePointer[a] = dataFile.readStringUntil('\n');
          ...
          if (switches.circumventTFT == 0) {
            tft.println(*globalReferencePointer[a]);
          }
        ...
        for (uint8_t a = newChip.pins / 2; a < newChip.pins; a++) //7-13
        {
          ...
          *globalpinFunctionPointer[a] = newChip.pinFunction;
          if (newChip.pinFunction == "Input  ") {
            pinMode(pin[a], OUTPUT);  //
            digitalWrite(pin[a], LOW);
            buttonsPinout[a].initButton(&tft, 160, nudge + 10, buttonSize, buttonSize, NAVY, BLUE, WHITE, "", 1);
            if (switches.circumventTFT == 0) {
              buttonsPinout[a].drawButton();
            }
          }
          if (newChip.pinFunction == "Clock  ") {
            clock[a] = 1;   //
            buttonsPinout[a].initButton(&tft, 160, nudge + 10, buttonSize, buttonSize, BLUE, PURPLE, WHITE, "C", 2);
            if (switches.circumventTFT == 0) {
              buttonsPinout[a].drawButton();
            }
          }
          if (newChip.pinFunction == "Output ") {
            outputs[a] = 2;
            tablet.sortedOutputs[a] = 2;
            if (switches.circumventTFT == 0) {
              tft.fillCircle(160, nudge + 9, outputSize, DARKGREY);
            }
          }
          if (newChip.pinFunction == "GND    ") {
            pinMode (pin[a], OUTPUT);  //
            digitalWrite(pin[a], LOW);
            if (switches.circumventTFT == 0) {
              tft.fillRect(150 + horizontalNudge, nudge , boxSize, boxSize, GREEN);
            }
          }
          if (newChip.pinFunction == "VCC    ") {
            pinMode (pin[a], OUTPUT);  //
            digitalWrite(pin[a], HIGH);
            if (switches.circumventTFT == 0) {
              tft.fillRect(150 + horizontalNudge, nudge , boxSize, boxSize, RED);
            }
          }
          if (newChip.pinFunction == "NC     ") {
            if (switches.circumventTFT == 0) {
              tft.fillRect(150 + horizontalNudge, nudge , boxSize, boxSize, NAVY); //
            }
          }
          if (newChip.pinFunction == "In/Out ") {
            inOut[a] = 1;  //
            pinMode(pin[a], OUTPUT);
            digitalWrite(pin[a], LOW);
            buttonsPinout[a].initButton(&tft, 160, nudge + 10, buttonSize, buttonSize, WHITE, DARKGREY, WHITE, "", 1);
            if (switches.circumventTFT == 0) {
              buttonsPinout[a].drawButton();
            } *globalpinFunctionPointer[a] = "Input  ";
          }
...
void muxIOswitchRefresh()
{
  ...
  if (*pinNumberRoutingPointer == 14) {
    nudge = 30;  //So the pins are evenly spaced depending on number of pins on each chip
    pinSpacer = 30;
    shifterLeft = 9;
    shifterRight = 459;
  }
  if (*pinNumberRoutingPointer == 16) {
    nudge = 20;
    pinSpacer = 28;
    shifterLeft = 1;
    shifterRight = 476;
  }
  if (*pinNumberRoutingPointer == 20) {
    nudge = 13;
    pinSpacer = 26;
    shifterLeft = -3;
    shifterRight = 542;
  }
  if (*pinNumberRoutingPointer == 24) {
    nudge = 13;
    pinSpacer = 22;
    shifterLeft = 0;
    shifterRight = 550;
  ...
  {
    if (inOut[a] == 1) {
      buttonsPinout[a].initButton(&tft, 80, nudge + 10, buttonSize, buttonSize, BLACK, BLACK, BLACK, "", 1);  //erase previous graphics
      buttonsPinout[a].drawButton();
      *globalpinFunctionPointer[a] = "Input  ";
    }
    if (outIn[a] == 1) {
      tft.fillCircle(80, nudge + 9, outputSize, BLACK);  // erase previous graphics
      *globalpinFunctionPointer[a] = "Output  ";
    }
    if (inOut[a] == 1) {
      buttonsPinout[a].initButton(&tft, 0, 0, 0, 0, BLACK, BLACK, BLACK, "", 1);  //here the Com ports are inputs
      outputs[a] = 1;
      tablet.sortedOutputs[a] = 1;
      tft.fillCircle(80, nudge + 9, outputSize, DARKGREY);
      *globalpinFunctionPointer[a] = "Output  ";
    ...
    if (outIn[a] == 1) {
      buttonsPinout[a].initButton(&tft, 0, 0, 0, 0, BLACK, BLACK, BLACK, "", 1);  //
      buttonsPinout[a] = buttonsClearPinout[0];
      outputs[a] = 0;
      tablet.sortedOutputs[a] = 0;
      pinMode(pin[a], OUTPUT);
      digitalWrite(pin[a], LOW);
      buttonsPinout[a].initButton(&tft, 80, nudge + 10, buttonSize, buttonSize, WHITE, DARKGREY, WHITE, "", 1);
      buttonsPinout[a].drawButton();
  ...
  {
    if (inOut[a] == 1) {
      buttonsPinout[a].initButton(&tft, 160, nudge + 10, buttonSize, buttonSize, BLACK, BLACK, BLACK, "", 1);  //erase previous graphics
      buttonsPinout[a].drawButton();
      *globalpinFunctionPointer[a] = "Input  ";
    }
    if (outIn[a] == 1) {
      tft.fillCircle(160, nudge + 9, outputSize, BLACK);  // erase previous graphics
      *globalpinFunctionPointer[a] = "Output  ";
    }
    if (inOut[a] == 1) {
      buttonsPinout[a].initButton(&tft, 0, 0, 0, 0, BLACK, BLACK, BLACK, "", 1);   //here the Com ports are inputs
...
void demuxIOswitchRefresh()
{
  ...
  for (uint8_t a = *pinNumberRoutingPointer / 2; a < *pinNumberRoutingPointer; a++) //7-13
  {
    ...
    if (inOut[a] == 1) {
      tft.fillCircle(160, nudge + 9, 10, BLACK);  // erase previous graphics
      *globalpinFunctionPointer[a] = "Output  ";
    }
    if (outIn[a] == 1) {
      buttonsPinout[a].initButton(&tft, 0, 0, 0, 0, BLACK, BLACK, BLACK, "", 1);   //here the Com ports are inputs
...
void clockAndmuxButtons()
{
  ...
  if (switches.clockmenuToggle == 1)
  {
    ...
    {
      if (switches.clockbuttonStatus == 0) {
        switches.clockToggle = 1;  //Clock button ON
        buttonsMenus[2].drawButton(true);
      }
      if (switches.clockbuttonStatus == 1) {
        switches.clockToggle = 0;  //Clock button OFF
        buttonsMenus[2].drawButton();
      }
    }
    if (buttonsMenus[2].justPressed())
    {
      if (switches.lastclockbuttonStatus == 0) {
        switches.clockbuttonStatus = 1;  //flip the bit for toggle effect
      }
      if (switches.lastclockbuttonStatus == 1) {
        switches.clockbuttonStatus = 0;  //flip the bit for toggle effect
      }
    }
    if (buttonsMenus[3].justPressed())//truth table screen
    {
  ...
  if (switches.muxdemuxMenuToggle == 1)
  {
    if (buttonsMenus[2].justPressed())
    {
      if (switches.muxdemuxToggleStatus == 0) {
        switches.muxdemuxToggle = 1;  //Mux Demux button ON
        switches.muxdemuxFlag = 1;
        buttonsMenus[2].drawButton(true);
      }
      if (switches.muxdemuxToggleStatus == 1) {
        switches.muxdemuxToggle = 0;  //Mux Demux button OFF
        switches.muxdemuxFlag = 1;
        buttonsMenus[2].drawButton();
      }
      if (switches.lastmuxdemuxToggleStatus == 0) {
        switches.muxdemuxToggleStatus = 1; //flip the bit for toggle effect
      }
      if (switches.lastmuxdemuxToggleStatus == 1) {
        switches.muxdemuxToggleStatus = 0; //flip the bit for toggle effect
      }
    }
